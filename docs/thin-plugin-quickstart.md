...
1. The first run cold-starts the GPU, which takes a few minutes. While it starts you see one line,
   `⠋ 1m46s dreamscale: starting compute (a few mins)`. When output is not a terminal, you get plain
   startup-stage lines and a "still starting" line every 30 s instead. A final line says the
   session is ready.
